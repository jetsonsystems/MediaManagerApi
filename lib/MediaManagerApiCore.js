...
var Images = new Class({
  ...
  index: function(options) {
    log.info('Images.index: Indexing for path - %s',+ this.path);
    options = options || {};
    var that = this;
    ...
    imageService.index(indexOptions,function(err, result) {
      var status = 200;
      if (err) {
        log.info('Images.index', 'error from image service - ' + err);
        status = 500;
        options.errorCode = -1;
  ...
  read: function(id, options) { 
    ...
    var that = this;
    imageService.show(id.replace('$', ''), null, function(err, result) {
      var status = 200;
      if (err) {
  ...
  delete: function(id, options) {
    ...
    if(id){//delete single image
      imageService.deleteImage(id,function(err, result) {
        var status = 200;
        if (err) {
          log.info('Images.delete', 'error from image service - ' + err);
          status = 500;
          options.errorCode = -1;
          options.errorMessage = err;
        }
    ...
    if(options && options.query && options.query && options.query.trashState){
      if(options.query.trashState==="in"){
        imageService.emptyTrash(function(err, result) {
          var status = 200;
          if (err) {
            log.info('Images.delete', 'error from image service - ' + err);
            status = 500;
...
var Tags = new Class({
  ...
  index: function(options) {
    ...
    imageService.tagsGetAll(function(err, result) {
      var status = 200;
      if (err) {
        log.info('Tags.index', 'error from image service - ' + err);
        status = 500;
        options.errorCode = -1;
        options.errorMessage = err;
      }
...
var Tagger = new Class({
  ...
  index:function (options) {
    ...
    imageService.tagsGetAll(function (err, result) {
      var status = 200;
      if (err) {
        log.info('Tagger.index', 'error from image service - ' + err);
        status = 500;
        options.errorCode = -1;
        options.errorMessage = err;
      }
  ...
  create:function (attr, options) {
    ...
    if (isValid) {
      ...
      if (_.has(attr, 'add')) {
        ...
        imageService.tagsAdd(listOfImages, listOfTags,
          function (err, result) {
            var status = 200;
            if (err) {
              log.info('Tagger.add: Error in imageService.tagsAdd - %j', err);
              status = 500;
              options.errorCode = -1;
              options.errorMessage = err;
            }
      ...
      if(_.has(attr, 'replace')){
        ...
        imageService.tagsReplace(listOfImages, oldTags,newTags,
          function (err, result) {
            var status = 200;
            if (err) {
              log.info('Tagger.replace: Error in imageService.tagsReplace - %j', err);
              status = 500;
              options.errorCode = -1;
              options.errorMessage = err;
            }
      ...
      if(_.has(attr, 'remove')){
        ...
        var tagsToRemove = attr.remove.tagsToRemove;
        //var newTags = attr.replace.newTags;
        imageService.tagsRemove(listOfImages, tagsToRemove,
          function (err, result) {
            var status = 200;
            if (err) {
              log.info('Tagger.remove: Error in imageService.tagsRemove - %j', err);
              status = 500;
              options.errorCode = -1;
              options.errorMessage = err;
            }
...
var Importers = new Class({
      ...
      try {
        ...
        imageService.importBatchFs(
          ...
          function(err, importBatch) {
            var status = 200;
            if (err) { 
              log.info('Importers.create: Error saving image (1) - %j', err);
              status = 500;
              options.errorCode = -1;
              options.errorMessage = err;
            }
        ...
        log.info('Importers.create: Error saving image (2) - %s', err);
        status = 500;
        options.errorCode = -1;
        options.errorMessage = err;
        that.doCallbacks(status,
  ...
  index: function(options) {
    ...
    imageService.importBatchFindRecent(numBatches,
                                       ...
                                       function(err, result) {
                                         var status = 200;
                                         if (err) {
                                           log.info('Importers.index: error from image service - %s', err);
                                           status = 500;
                                           options.errorCode = -1;
                                           options.errorMessage = err;
                                         }
  ...
  read: function(id, options) {
    ...
    imageService.importBatchShow(id.replace('$', ''),
                                 ...
                                 function(err, result) {
                                   var status = 200;
                                   if (err) {
                                     log.info('Importers.show: error from image service - %s', err);
                                     status = 500;
                                     options.errorCode = -1;
                                     options.errorMessage = err;
                                   }
  ...
  //
  transformRep: function(rep, options) {
    log.info('Importers.transformRep: Doing transform, path - %s', this.path);
    var that = this;
    if (options && _.has(options, 'isInstRef') && options.isInstRef) {
      log.info('Importers.transformRep: transforming instance to full form...');
...
var ImportersImages = new Class({
  ...
  index: function(options) {
    ...
    imageService.importBatchShow(batchId.replace('$', ''),
                                 ...
                                 function(err, result) {
                                   var status = 200;
                                   if (err) {
                                     log.info('ImportersImages.index: error from image service - %s', err);
                                     status = 500;
                                     options.errorCode = -1;
                                     options.errorMessage = err;
                                   }
  ...
  transformRep: function(rep, options) {
    ...
    if (newRep) {
      if (rep.images) {
        log.info('ImportersImages.transformRep: Doing image transform - %j ', rep.images);
...
var StorageSynchronizers = new Class({
  ...
  create: function(attr, options) {
    ...
    try {
      ...
      log.info('StorageSynchronizers.create: Caught error - %s', err);
      options.errorCode = -1;
...
var StorageChangesFeed = new Class({
  ...
  create: function(attr, options) {
    ...
    try {
      ...
      log.info('StorageChangesFeed.create: Caught error - %s', err);
      options.errorCode = -1;
      options.errorMessage = err.message;
      try {
...
function buildIndexOptions(options){
  ...
  if(options.query && options.query.tags){
